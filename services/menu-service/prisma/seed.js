...
const { PrismaClient, MealTemplateKind, MealItemRole, DayType } = require("@prisma/client");
...
async function seedFoodItems() {
  ...
  // נחזיר map לפי שם לשימוש נוח בהמשך
  const map = new Map();
...
async function createMealTemplate(prismaTx, { name, kind, coachId, items }) {
  // מחשבים totalCalories על בסיס הפריטים
  let totalCalories = 0;
...
async function seedTemplateMenus(foodMap) {
  ...
  // נגדיר מבנה ייעודי לנתוני seed, לפי שתי התמונות שלך
  const templatesData = [
    {
      name: "יום ללא אימון",
      ...
      meals: [
        {
          name: "ארוחה 1",
          // כאן לפי התמונה זו בעיקר ארוחת חלבון+פחמימה, נייצג אותה כאופציה יחידה
          options: [
            {
              name: "אופציה בסיסית",
    ...
    for (const mealDef of template.meals) {
      ...
      for (let i = 0; i < mealDef.options.length; i++) {
        ...
        if (!firstOptionId) {
          ...
          // מוסיפים את הקלוריות של האופציה הראשונה לסה"כ התפריט
